        ...
        def div_stress_s4f(U, alpha=1):
            ...
            for i, cell in enumerate(s.mesh.cells): # Iterate through cells                
                ...
                for j, face in cell.stencil.items():
                    ...
                    grad_Ux_face = (grad_Ux[i] + grad_Ux[j]) / 2
                    grad_Uy_face = (grad_Uy[i] + grad_Uy[j]) / 2
                    ...
                    traction[0] += alpha * Kbar * ((Ux[j] - Ux[i])/proj_distance - grad_Ux_face[0] * normx - grad_Ux_face[1] * normy)
                    traction[1] += alpha * Kbar * ((Uy[j] - Uy[i])/proj_distance - grad_Uy_face[0] * normx - grad_Uy_face[1] * normy)
                ...
                for b, face in cell.bstencil.items():
                    ...
                    if cdt_type == 'displacement' or cdt_type == 'Displacement':
                        ...
                        # Add the Rhie-Chow term to the traction
                        traction[0] += alpha * Kbar * ((Ux_b - Ux[i])/proj_distance - grad_Ux[i][0] * normx - grad_Ux[i][1] * normy)
                    ...
                    if cdt_type == 'displacement' or cdt_type == 'Displacement':
                        ...
                        # Add the Rhie-Chow term to the traction
                        traction[1] += alpha * Kbar * ((Uy_b - Uy[i])/proj_distance - grad_Uy[i][0] * normx - grad_Uy[i][1] * normy)
